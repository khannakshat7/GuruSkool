...
def teacherRegister(request):
    ...
    if(request.method == 'POST'):
        ...
        if user_form.is_valid():
            ...
            msg={"message":True}
            return render(request,"login.html",msg) # Redirect to login page if registration is successful
        else:
            error={"error":True}
            return render(request,"register.html",error) # Redirect to register page if registration is unsuccessful
    else:
        return render(request, 'register.html')
...
def loginUser(request):
    if(request.method == 'POST'):
        ...
        if user:
            ...
            return render(request,"login.html",{"invalid":True}) # Redirect to login page if user credentials are invalid
    else:
...
def createClassSlot(request):
    if request.method == 'POST':
        ...
        class_slot_details.description = request.POST.get('class_slot_description')
        class_slot_details.date = request.POST.get('class_slot_date')
        class_slot_details.start_time = request.POST.get('class_slot_start_time')
        ...
        subject = "New In-Person class Slot | "+ class_slot_details.name +" | "+ class_slot_details.start_time +" to "+ class_slot_details.end_time + " | " + class_slot_details.date 
        message = "Hi,\n\n" + "A new class slot has been opened for slot booking.\n\n" + "Name: " + class_slot_details.name + "\n" + "Description: " + class_slot_details.description + "\n" + "Date: " + class_slot_details.date + "\n" + "Start Time: " + class_slot_details.start_time + "\n" + "End Time: " + class_slot_details.end_time + "\n" + "Link: " + class_slot_details.link + "\n\n" + "Regards,\n" + "गुरुSkool Team"
        all_students = User.objects.filter(is_superuser=False)
        for student in all_students:
            send_mail(subject,message,settings.EMAIL_HOST_USER,[student.username],fail_silently=False)
...
def viewClassSlots(request):
    try:
        ...
        class_slots = None
    return render(request, 'class_slots.html', {'class_slots': class_slots}) # Render class slots page with all class slots
...
def editClassSlot(request, class_slot_id): # Get class slot id from url
    ...
    if request.method == 'POST':
        ...
        return render(request, 'create_class_slot.html', {'class_slot': class_slot}) # Render create class slot page with class slot details

# This view is used to delete Class slot which are created by faculty
@login_required(login_url='/login/')
@permission_required('is_superuser',raise_exception=True)
def deleteClassSlot(request, class_slot_id):
    class_slot = ClassSlotDetails.objects.get(id=class_slot_id) # Get class slot id from url
...
def bookClassSlot(request):
    if request.method == 'POST':
        ...
        class_slot_id = request.POST.get('class_slot_id') # Get class slot id from form
        ...
        if(class_details.current_capacity>1):
            ...
            send_mail(subject, message,settings.DEFAULT_FROM_EMAIL, [request.user.username])
